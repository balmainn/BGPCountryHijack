...
# while len(allUpdates)<10:
# print('allupdates ',cnt)
#startTime = '2024-04-30T03:30:00'
tslist = []
someNeighbors = set()
#while len(someNeighbors) <7:
numFiles = 3 #(30 mins for rv 10 for rrc)
...
def descreteSplit(allUpdates,fudge=0):
    lastSplitIndex = 0
    tdiffs = findGreatestDiff(allUpdates)
    split = tdiffs[0+fudge]#tdiffs[int(len(tdiffs)/2)] #1
    splitUpdates = []
    for i in range(len(allUpdates)):  
        update1 = allUpdates[i]
        try:
            ...
            break  
        tdiff = update2['timestamp']-update1['timestamp']
        if tdiff >=split-0.05 and tdiff <= split +0.05:            
            splitUpdates.append(allUpdates[lastSplitIndex:i])
            lastSplitIndex = i
    print(lastSplitIndex,len(allUpdates))
    if lastSplitIndex == 0:
        return descreteSplit(allUpdates,fudge+1)
    if lastSplitIndex < len(allUpdates):
        splitUpdates.append(allUpdates[lastSplitIndex:])            
    return splitUpdates

def splitSplit(allUpdates,start,end,splitLen):
    splits = []
    lastSplitIndex= start
    for i in range(start,end,1):
        ...
        try:
            ...
            break  
        tdiff = update2['timestamp']-update1['timestamp']
        if tdiff >=splitLen-0.05 and tdiff <= splitLen +0.05:            
            splits.append(((lastSplitIndex,i)))
            lastSplitIndex = i
    if lastSplitIndex < end:
        splits.append((lastSplitIndex,end))    
    return splits
def fullSplit(allUpdates):

    tdiffs = findGreatestDiff(allUpdates)
    split = tdiffs[int(len(tdiffs)/2)] #1
    splits = []
    lastSplits= []
    for t in range(len(tdiffs)):
        ...
        tdiff = tdiffs[t]
        if len(splits)==0:
            splits = splitSplit(allUpdates,0,len(allUpdates),tdiff)
        else:
            for start,end in splits:
                splits = splitSplit(allUpdates,start,end,tdiff)
            lastSplits.append(splits)
    for l in lastSplits:
        for s in l:
            print(s)                  
    return splits
#fullSplit(allUpdates)
#exit(0)
numTDiffs = findNumTdiffs(allUpdates)
someList = []
for tdiff in numTDiffs:
    num = numTDiffs[tdiff]
    someList.append((tdiff,num))
for tdiff,num in someList:
    if num > 10:
        print(tdiff,num)
#print(sorted(someList, key=lambda x: x[1],reverse=True)[0:5],sorted(someList, key=lambda x: x[1],reverse=True)[-5:])
#exit(0)
desUpdates = descreteSplit(allUpdates)

print('des bucket lens')
for bucket in desUpdates:
    print(len(bucket))
# input()
#exit(0)    
buckets = desUpdates
...
while recurse :
    ...
    for singleBucket in buckets:
        ...
        if len(newBuckets)>0:
            try:
                buckets.remove(singleBucket)
            except Exception as E:
                print('newbucket exception',E)
                #part of the original set, so its probably fine
                pass
        ...
        if noContras:
            if len(values)>0:
                ...
                if not badChange:
                    ...
                    splitBuckets = descreteSplit(singleBucket)
                    #splitBuckets = splitUpdates(singleBucket)
                    for s in splitBuckets:
                        if len(s) ==0:
                            continue
                        newBuckets.append(s)
                    #print(len(splitBuckets[0]),len(splitBuckets[1]))
