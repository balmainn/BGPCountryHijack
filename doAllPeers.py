...
args = []
# for peer_asn,peer_ip in ripePeers[collector]['peers']:
#     args.append((startTime,endTime,collector,peer_ip))
    
#arbMultiProcessing.goManageArgs(helperFunc,args,10)
#print(ripePeers[collector]['peers'])
#print(len(print(ripePeers[collector]['peers'])))
# print(len(rvPeers[collector]['ipv4']))
        #numIPs,peerASN,peerIP
# exit(0)
allUpdates = []
maxUpdates = -1
#depricated
...
for peer_asn,observerIP in ripePeers['rrc04']['peers']:
    ...
    print(combined)
    allCombined.append((combined,done))
for i in range(len(allPeers)):
    print(allPeers[i],allCombined[i],len(allCombined[i][0]))
